...
function AutoRotatingSlideshow({
  photos,
  onImageClick,
}: {
  photos: string[];
    ...
    return (
      <div className="relative">
        <div className="relative overflow-hidden rounded-xl shadow-md">
          <div
            className={`flex ${
              photos.length === 1 ? "justify-center" : "justify-between"
            } gap-3`}
          >
            {photos.map((photo, idx) => (
              <motion.div
                ...
                style={{
                  width:
                    photos.length === 1 ? "100%" : `calc((100% - 0.75rem) / 2)`,
                  aspectRatio: "4/3",
                }}
...
export default function Events() {
  ...
  const upcomingEvents = useMemo(
    () => events.filter((e) => !e.breaking && new Date(e.date) >= today),
    [events, today]
  );
  ...
  const displayedEvents =
    eventType === "upcoming" ? upcomingEvents : completedEvents;
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-white/80 text-sm mt-1">
                    Stay updated with our latest activities and gatherings
                  </p>
                </div>
              </div>
            </div>
                        ...
                        <div className="mt-6">
                          <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-xs font-medium text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                              {e.photos.length}{" "}
                              {e.photos.length === 1 ? "Photo" : "Photos"}
                            </span>
                          </div>
                          <AutoRotatingSlideshow
                            photos={e.photos}
                            onImageClick={setOpenImg}
                          />
                        </div>
                      )}
