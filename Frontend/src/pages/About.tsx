...
import { getTotals, getAboutMilestones, getPastEvents } from "../services/api";
...
export default function About() {
  ...
  useEffect(() => {
    try {
      localStorage.removeItem("crea_past_events");
      localStorage.removeItem("crea_past_events_removed_defaults");
    } catch (e) {
  ...
  // Check if milestones are loaded
  const milestonesLoaded = timelineStops.length > 0;
  const currentStop = milestonesLoaded ? timelineStops[activeStop] : null;

  // Gallery state that fetches from database
  ...
  useEffect(() => {
    ...
    const handler = () => loadGalleryFromDatabase();
      ...
      <div className="relative bg-gradient-to-br from-[#0a2343] via-[var(--primary)] to-[#0a2343] rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-12 overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="text-center mb-8 sm:mb-10 md:mb-12">
            <h2
              className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-2 sm:mb-3 md:mb-4 px-2"
              style={{
                color: "#ffffff",
                      ...
                      >
                        <ChevronLeftIcon
                          className={`w-5 h-5 sm:w-6 sm:h-6 transition-transform ${
                            activeStop !== 0 && "group-hover:-translate-x-1"
                          }`}
                        />
                      </button>
                      <button
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6">
          {galleryLoaded && gallery.length === 0 ? (
            <div className="col-span-2 md:col-span-3 text-center py-12 text-gray-500">
